"""Business news source adapter for strategic finance opportunities."""
...
def collect_business_news_opportunities(records: list[dict[str, str]] | None = None) -> list[Opportunity]:
    """Normalize business-news items into finance opportunities.
    ...
    for item in records:
        ...
        normalized.append(
            Opportunity(
                ...
                initiative_type=item.get("initiative_type", "M&A / Integration"),
                trigger_date=date.fromisoformat(source_date),
                publication_date=date.fromisoformat(source_date),
                timeline=item.get("timeline", "TBD"),
                summary=item.get("summary", "Business news suggests strategic finance transformation trigger."),
                role_we_could_fill=item.get("role_we_could_fill", "M&A Integration"),
