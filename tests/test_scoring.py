...
def test_recency_bucket_boundaries() -> None:
    ...
    assert recency_bucket(today - timedelta(days=10), today).value == "0–30 days"
    assert recency_bucket(today - timedelta(days=60), today).value == "31–90 days"
    assert recency_bucket(today - timedelta(days=120), today).value == ">90 days"
